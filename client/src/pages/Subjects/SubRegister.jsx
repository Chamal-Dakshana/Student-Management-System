...
export default function SubRegister() {
  ...
  const registerSubject = async(e) => {
    ...
    try {
      const {data} =await axios.post('/subregister', {
        name, email, password
      })
      if(data.error){
    ...
    return(
      <div className='continar'>
        ...
        <div className='topic'>SUBJECT SIGN UP</div>
            <form onSubmit={registerSubject}>
              <div className='inputs'>
                <div className='input'>
                  <img className='Img1' src={student_icon} alt="" />
                  <TextField className='text1' type='text' id="filled-basic" label="Enter the Subject name" variant="filled" value={data.name} onChange={(e) => setData({...data, name:e.target.value})}/>
                </div>
                <div className='input'>
